...
export function SeatMapCreator() {
  ...
  const [selectedZone, setSelectedZone] = useState(
    palenqueData.zones && palenqueData.zones.length > 0
      ? palenqueData.zones[0].id
      : "default",
  );
  ...
  const rowOptions = Array.from({ length: 26 }, (_, i) =>
  ...
  const getZoneInfo = (zoneId: string) => {
    return (
      palenqueData.zones.find((z) => z.id === zoneId) || palenqueData.zones[0]
    );
  };
  ...
  // Generar ID único
  const generateId = () =>
    `seat_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  const generateQuadraticBezier = (
    ...
    const controlX =
      midX + Math.cos(perpAngleRad) * controlDistance * Math.sin(angleRad);
    const controlY =
      midY + Math.sin(perpAngleRad) * controlDistance * Math.sin(angleRad);
  ...
  const getNextSeatNumber = (rowLetter: string): number => {
    const seatsInRow = createdSeats.filter(
      (seat) => seat.rowLetter === rowLetter,
    );
  ...
  const handleCanvasClick = (event: React.MouseEvent<SVGSVGElement>) => {
    ...
    if (creationMode === "single") {
      ...
      console.log(
        `Manejando línea en: (${x}, ${y}), isCreatingLine: ${isCreatingLine}`,
      );
  ...
  const handleLineCreation = (x: number, y: number) => {
    if (!isCreatingLine) {
      ...
      toast({
        ...
        description:
          "Haz clic y arrastra para crear la línea curva de asientos",
      });
    }
  };
  ...
  const handleMouseMove = (event: React.MouseEvent<SVGSVGElement>) => {
    if (creationMode === "line" && isCreatingLine && lineStartPoint) {
      ...
      if (isDraggingCurve && lineEndPoint) {
        ...
        setPreviewSeats(seats);
        setPreviewCurvePath(
          generateCurvePath(lineStartPoint, lineEndPoint, angle),
        );
      } else {
  ...
  const deleteSelectedSeats = () => {
    setCreatedSeats((prev) =>
      prev.filter((seat) => !selectedSeats.includes(seat.id)),
    );
    setSelectedSeats([]);
    toast({
  ...
  const deleteLine = (lineId: string) => {
    ...
    setSelectedSeats((prev) =>
      prev.filter(
        (id) =>
          !createdSeats.find(
            (seat) => seat.id === id && seat.lineId === lineId,
          ),
      ),
    );
  };
  ...
  const updateLineCoordinates = () => {
    ...
    const lineSeats = createdSeats.filter(
      (seat) => seat.lineId === editingLine.id,
    );
  ...
  const visibleSeats = useMemo(() => {
    return createdSeats.filter((seat) =>
      isSeatInViewport(seat, currentViewBox),
    );
      ...
      <div className="w-96 bg-white shadow-xl overflow-y-auto">
        ...
        <div className="p-6 border-b">
          <h1 className="text-xl font-bold text-gray-900 mb-2">
            Creador de Mapas de Asientos
          </h1>
          <p className="text-gray-600 text-sm">
            Haz clic en el canvas para agregar asientos
          </p>
        </div>
          ...
          <div className="p-6 border-b">
            ...
            {editingMode === null && (
              <div className="text-sm text-gray-600 p-3 bg-blue-50 rounded-lg">
                Haz clic en un asiento para editarlo. Si pertenece a una línea,
                podrás editar toda la línea.
              </div>
            )}
            ...
            {editingMode === "seat" && editingSeat && (
              <div className="space-y-4">
                <h4 className="font-medium text-gray-700">
                  Editando Asiento {editingSeat.rowLetter}
        ...
        <div className="p-6 border-b">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={selectedRowLetter}
                onValueChange={setSelectedRowLetter}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            {creationMode === "line" && (
              <div className="space-y-2">
                ...
                <p className="text-xs text-gray-500">
                  Los asientos se numerarán consecutivamente en la fila{" "}
                  {selectedRowLetter}
                </p>
              </div>
            )}
        ...
        <div className="p-6">
          ...
          <div className="mt-6 text-xs text-gray-500 space-y-1">
            <p>
              • Área válida: {(ruedo.radius * 2 + 50).toFixed(0)}px -{" "}
              {(maxRadius - 50).toFixed(0)}px del centro
            </p>
            <p>
            ...
            {selectedSeats.length > 0 && (
              <p>• Seleccionados: {selectedSeats.length}</p>
            )}
                ...
                {visibleSeats.map((seat) => (
                  <g key={seat.id}>
                    <circle
                      ...
                      stroke={
                        selectedSeats.includes(seat.id) ? "#BE185D" : "#374151"
                      }
                      strokeWidth="1.5"
